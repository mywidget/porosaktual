...
@section('content')
<form method="POST" action="{{ route('admin.advertisements.update', $advertisement) }}" enctype="multipart/form-data" x-data="adForm()">
    @csrf @method('PUT')

    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Iklan</h1>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-700">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="slot_id" id="slot_id" required class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            @foreach($slots ?? [] as $slot)
                                <option value="{{ $slot->id }}" {{ old('slot_id', $advertisement->slot_id) == $slot->id ? 'selected' : '' }}>{{ $slot->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $advertisement->title) }}" required class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="type" id="type" x-model="type" required class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <option value="html_script" {{ old('type', $advertisement->type) === 'html_script' ? 'selected' : '' }}>HTML Script</option>
                            <option value="internal" {{ old('type', $advertisement->type) === 'internal' ? 'selected' : '' }}>Internal</option>
                        </select>
                    </div>
                    ...
                    <div x-show="type === 'banner'" x-transition>
                        ...
                        @if($advertisement->banner_image)
                            <div class="mb-2">
                                <img src="{{ asset('storage/' . $advertisement->banner_image) }}" alt="" class="h-20 rounded border border-gray-200 dark:border-gray-600">
                            </div>
                        @endif
                    ...
                    <div x-show="type === 'html_script'" x-transition>
                        ...
                        <textarea name="html_code" id="html_code" rows="8" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 font-mono text-sm">{{ old('html_code', $advertisement->html_code) }}</textarea>
                    </div>

                    <div>
                        <label for="url" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">URL Tujuan</label>
                        <input type="url" name="url" id="url" value="{{ old('url', $advertisement->url) }}" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-700">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="date" name="start_date" id="start_date" value="{{ old('start_date', optional($advertisement->start_date)->format('Y-m-d')) }}" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>

                    <div>
                        <label for="end_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tanggal Berakhir</label>
                        <input type="date" name="end_date" id="end_date" value="{{ old('end_date', optional($advertisement->end_date)->format('Y-m-d')) }}" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>

                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $advertisement->is_active) ? 'checked' : '' }} class="rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500">
                        <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Aktif</span>
                    </label>
                </div>
            </div>

            <button type="submit" class="w-full px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition">Update Iklan</button>
        </div>
    </div>
</form>
...
<script>
    function adForm() {
        return {
            type: '{{ old("type", $advertisement->type) }}'
        }
    }
</script>
@endpush
